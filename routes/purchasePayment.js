...
//   taxDescription = invData[i].taxDescription;//
   ...
   // if (invData[i].payAmount < 0) {
   //     invData[i].payAmount = Math.abs(invData[i].payAmount)
  //  }
   drAmt = parseFloat(Math.abs(invData[i].payAmount));;
   crAmt = 0.00;
   taxTotal = 0.00;
   noteTaxTotal += 0.00;
...
console.log(dbquery);
    ...
    con.query(dbquery, function(err, row) {
                        ...
                        if (err) {
                         console.error("❌ invoiceTxn insert  error:", err);
