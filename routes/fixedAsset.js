...
router.get('/', async (req, res) => {
  try {
    ...
    // 1. Load all fixed asset accounts (glType 402 or 406)
    const [accounts] = await db.execute(
      `SELECT * 
       FROM glAccount 
       WHERE companyID = ? 
         AND (glType='402' OR glType='406') 
       ORDER BY glNo, glSub`,
      [companyID]
    );
    ...
    for (const account of accounts) {
      ...
      // Sum opening balance from journal before startDate
    ...
    console.error("Error in fixedAsset route:", err);
    res.status(500).send("Failed to load fixed asset data");
  }
});
