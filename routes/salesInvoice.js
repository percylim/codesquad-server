...
var txnDate = moment(invData[0].txnDate).format('YYYY-MM-DD');
var dueDate = moment(invData[0].dueDate).format('YYYY-MM-DD');
...
let hasError = false;
 ...
 for (let i = 0; i < invData.length; i++) {
   ...
   if (typeof invData[i].cost === 'string') {
       invData[i].cost = parseFloat(invData[i].cost); 
   }
   totalCost+= invData[i].cost;
                          ...
                          // create new record
dbquery = "INSERT INTO salesInvoice (companyID, customerID, customerName, invoiceNo, invoiceDate, productID, barcode, productName, productDescription, unit, salesQty, unitPrice,  itemDiscount, taxID, taxType, taxCode, taxRate, itemTotal, taxItemTotal, itemNetTotal, voucherNo, jvInit, dueDate, salesRep, remark1, remark2, remark3, remark4, remark5, remark6, txnDate, itemTax, salesQuantity, cost,date_created) VALUE('" + companyID + "', '"+ customerID + "', '"+ customerName + "', '"+ invoiceNo + "', '"+ txnDate + "', '"+productID+"', '"+barcode+"', '"+productName+"', '"+productDescription+"', '"+unit+"', '"+salesQty+"', '"+unitPrice+"', '"+itemDiscount+"', '"+taxID+"', '"+taxType+"', '"+taxCode+"', '"+taxRate+"', '"+itemTotal+"', '"+itemTax+"', '"+itemNetTotal+"', '"+voucherNo+"','"+jvInit+"', '"+dueDate+"','"+salesRep+"','"+remark1+"', '"+remark2+"', '"+remark3+"', '"+remark4+"', '"+remark5+"', '"+remark6+"', '"+txnDate+"', '"+itemTax+"', '"+salesQty+"', '"+cost+"', CURDATE())"
 ...
 console.log(dbquery);
      ...
      con.query(dbquery, function(err, row) {
          if (hasError) return;
                          if (err) {
           hasError = true;
                            console.error('❌ G/L Account Info DB Error:', err);
