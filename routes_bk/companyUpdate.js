...
dbquery="UPDATE company SET registerNo = '"+registerNo+"', address1 = '"+address1+"', address2 = '"+address2+"', postcode='"+postcode+"', city='"+city+"', state='"+state+"', country='"+country+"', incomeTaxNo='"+incomeTaxNo+"', epfNo='"+epfNo+"', socsoNo='"+socsoNo+"', gstNo='"+gstNo+"', telNo1='"+telNo1+"', telNo2='"+telNo2+"', faxNo='"+faxNo+"', email='"+email+"', website='"+website+"', finYearStart='"+finYearStart+"', finYearEnd='"+finYearEnd+"' WHERE companyID='"+companyID+"'"

                  } else {   // create new data

                      dbquery="INSERT INTO company (companyID, companyName, registerNo, address1, address2, postcode, city, state, country, incomeTaxNo, epfNo, socsoNo, gstNo, telNo1, telNo2, faxNo, email, website, finYearStart, finYearEnd, date_created) VALUES('" + companyID + "','"+ companyName +"','"+ registerNo +"','"+ address1+ "','"+ address2+ "','"+ postcode+ "','"+ city+ "','"+ state+ "', '"+ country+ "', '"+ incomeTaxNo+ "', '"+ epfNo+ "', '"+ socsoNo+ "', '"+gstNo+ "', '"+ telNo1+ "', '"+ telNo2+ "', '"+ faxNo+ "', '"+ email+ "','"+ website+ "','"+ finYearStart + "', '"+ finYearEnd+ "', CURDATE() )" ;
                  }
