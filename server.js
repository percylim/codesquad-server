...
fs.readdir('./public/uploads', (err, files) => {
  if (err) console.error("Cannot read uploads folder", err);
  else console.log("Image files:", files);
});
...
const incomeTaxRouter = require('./routes/incomeTax');
...
const loadGlInfoRouter = require('./routes/loadGlInfo');
const glSaveRouter = require('./routes/glSave');
...
const allowedOrigins = [
  ...
  "https://stall-centralsoft.com.my",
  "https://stall.centralsoft.com.my",
  "https://food.stall-centralsoft.com",
  "http://localhost:3000"// for production
];


const corsOptions = {
  origin: ['http://localhost:3000', 'https://centralsoft.com.my', 'https://food.stall-centralsoft.com'],
  credentials: true,
...
app.use("/api/glSave", glSaveRouter);
...
app.use('/api/loadGlInfo', loadGlInfoRouter);
...
app.use("/api/productAdjustWriteOffSearch", productAdjustWriteOffSearchRouter);
...
// Explicit API 404 handler: any /api/* route not matched above returns JSON 404.
// Put this BEFORE the SPA catch-all so API unknown routes do NOT get index.html.
app.use('/api/*', (req, res) => {
  console.warn(`❌ Unknown API route: ${req.originalUrl}`);
  res.status(404).json({ success: false, message: 'API route not found' });
});

// Non-API routes and the SPA: serve index.html for client-side routing only.
// This regex matches any path that does NOT start with /api
app.get(/^\/(?!api).*/, (req, res) => {
  const indexPath = path.join(__dirname, '../frontend/build', 'index.html');
  console.log(`📦 Serving SPA index for ${req.originalUrl}`);
  res.sendFile(indexPath, (err) => {
    if (err) {
      console.error("Error sending SPA index.html:", err);
